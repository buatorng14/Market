...
if st.session_state.stage == 0:
    ...
    #ไปหน้าตะกร้าสินค้า   
    st.button(':shopping_trolley: ตะกร้าสินค้า', on_click=set_state, args=[1] ,use_container_width=True)
    #ปุ่มตรวจสอบสินค้า
    st.button('ตรวจสอบสถานะ',key=1,on_click=set_state, args=[5], use_container_width=True)
    # Search input
...
if st.session_state.stage == 1:
    ...
    with col1:
            st.button('ย้อนกลับ',key = 1, on_click=set_state, args=[0],use_container_width=True)
    with col2:        
        if item_quantity>0:
            st.button('ถัดไป',key='ถัดไป1', on_click=set_state, args=[2],use_container_width=True)
        elif item_quantity<=0:
            if st.button('ถัดไป',key='ถัดไป2', on_click=set_state,args=[1],use_container_width=True):
...
if st.session_state.stage == 2:
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.button('ย้อนกลับ', key=2, on_click=set_state, args=[1], use_container_width=True)
    with col2:
        if not name_customer:
            if st.button('SUBMIT',key='submit1', on_click=set_state, args=[2], use_container_width=True):
                st.warning('โปรดใส่ชื่อผู้สั่งซื้อด้วย')
        else:
            cursor.execute("SELECT * FROM Orders WHERE OrderCode=?", (name_customer,))
            result = cursor.fetchone()
            if result:
                if st.button('SUBMIT',key='submit3', on_click=set_state, args=[2], use_container_width=True):
                    st.warning('ชื่อนี้มีอยู่แล้ว กรุณาเปลี่ยนชื่อ')
            else:
                st.button('SUBMIT',key='submit2', on_click=set_state, args=[3], use_container_width=True)
...
if st.session_state.stage == 3:
    ...
    if order_data:
        ...
        col2.button('สถานะสินค้า', on_click=set_state, args=[4], use_container_width=True)
...
if st.session_state.stage == 4:
    ...
    if col2.button('ตรวจสอบสถานะอีกครั้ง',on_click=set_state, args=[4], use_container_width=True):
...
if st.session_state.stage == 5:
    ...
    if not name_chek :
            if col2.button('ตรวจสอบสถานะอีกครั้ง',key="chek1",on_click=set_state, args=[5], use_container_width=True):
                st.warning('โปรดใส่ชื่อผู้สั่งซื้อด้วย')
    else :
